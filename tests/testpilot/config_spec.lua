...
describe("testpilot.config", function()
  ...
  describe("defaults", function()
    ...
    it("has notify = failures", function()
      assert.are.equal("failures", config.get().notify)
    end)
  end)
  ...
  describe("apply", function()
    ...
    it("accepts notify = all", function()
      config.apply({ notify = "all" })
      assert.are.equal("all", config.get().notify)
    end)

    it("accepts notify = none", function()
      config.apply({ notify = "none" })
      assert.are.equal("none", config.get().notify)
    end)
  ...
  describe("reset", function()
    it("restores defaults after apply", function()
      config.apply({ open_method = "edit", notify = "none" })
      config.reset()
      assert.are.equal("vsplit", config.get().open_method)
      assert.are.equal("failures", config.get().notify)
    end)
